...
class Storage
{
    ...
    {
        if ($path === null) {
            $skillId = $this->getSkillId() ?? 'common';
            $path = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'alice_storage' . DIRECTORY_SEPARATOR . $skillId;
        }
    ...
    public function set(string $key, mixed $value, int $ttl = 0): static
    {
        ...
        if (file_put_contents($tmp, json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)) === false) {
